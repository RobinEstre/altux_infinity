...
@Component({
  selector: 'app-base-extra',
  templateUrl: './base-extra.component.html',
  styleUrls: ['./base-extra.component.scss'],
  providers: [ { provide: LOCALE_ID, useValue: 'es' }, DatePipe]
})
export class BaseExtraComponent implements OnInit {
  ...
  @ViewChild('modal_control') private modalContent: TemplateRef<BaseExtraComponent>;
  private modalRef: NgbModalRef;
  @ViewChild(DataTableDirective, {static: false})
  dtElement: DataTableDirective;

  @ViewChild('dtActions') dtActions!: TemplateRef<BaseExtraComponent>;
  @ViewChild('is_celular') is_celular!: TemplateRef<BaseExtraComponent>;
  @ViewChild('idTpl', {static: true}) idTpl: TemplateRef<BaseExtraComponent>;
  @ViewChild('is_tipo') is_tipo!: TemplateRef<BaseExtraComponent>;
  @ViewChild('is_alumno') is_alumno!: TemplateRef<BaseExtraComponent>;
  @ViewChild('is_vendedor') is_vendedor!: TemplateRef<BaseExtraComponent>;
  @ViewChild('is_categoria') is_categoria!: TemplateRef<BaseExtraComponent>;

  constructor(private spinner: NgxSpinnerService, private cd: ChangeDetectorRef,private datePipe: DatePipe, 
    private service: MarketingService,private modalService: NgbModal){
  }
  ...
  public paginate:any; public start_paginate:number=0; register_count:number; filter_params:any; detalle:any; leads:any=[]; repetidos:any=[]
  ...
  listInit(){
    ...
    this.dtOptions = {
      ...
      processing: true,
      searchDelay: 600,
      language: BaseExtraComponent.spanish_datatables,
  ...
  onCaptureEvent(event: any): void {
    ...
    if(event.data.checked){
      if(this.leads.length==0){
        this.leads.push(event.data.id)
      }else{
        this.leads.forEach(i => {
          if (!this.leads.includes(event.data.id)) {
            this.leads.push(event.data.id);
          }
        });
      }
    }else{
      this.leads = this.leads.filter(i => i != event.data.id);
    }
  }
  ...
  selectSeller(){
    if(this.leads.length==0){
      Swal.fire({
        position: "center",
        icon: "warning",
        title: "Leads No Seleccionado",
        showConfirmButton: false,
        timer: 2500
      });
    }else{
      this.service.listVendedores2().subscribe(resp=>{
        if(resp.success){
          let data:any=[]
          resp.data.forEach(i=>{
            data.push({
  ...
  chargeVendedores(data, info) {
    ...
    swalWithBootstrapButtons.fire({
      ...
      if (result.isConfirmed) {
        this.asignarSeller(+result.value, info)
        //console.log(result);
      }
    })
  }
  
  chargeVendedores2(data) {    
    var options = {};
    $.map(data,
    function(o) {
      options[o.id] = o.name;
    ...
    swalWithBootstrapButtons.fire({
      ...
      if (result.isConfirmed) {
        this.asignarSeller2(+result.value, data)
        //console.log(result);
      }
    })
  }
        ...
        this.service.asignarVendedores(this.detalle.id, body).subscribe(resp=>{
          if(resp.success){
            ...
            Swal.fire({
              position: "center",
              icon: "success",
              title: "Base Extra fue asignado correctamente",
              showConfirmButton: false,
              timer: 2500
            });
            this.rerender()
  ...
  asignarSeller2(id, data){
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.service.asignarVendedores2(body).subscribe(resp=>{
          if(resp.success){
            this.spinner.hide()
            Swal.fire({
              position: "center",
            ...
            this.repetidos=resp.repetidos_vendedor
            this.rerender()
            if(this.repetidos.length>0){
              this.openModal()
            }
          }
        })
